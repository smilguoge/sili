<template>
  <div class="app-container">
    <div class="filter-container flex">
      <div class="spsty1">
        单据号：
        <el-input v-model="listQuery.document_no" type="text" clearable placeholder="选择单据" style="width:90px;margin-right:15px;" />
      </div>
      <div class="spsty1">
        日期：
        <el-date-picker v-model="start_time" type="date" placeholder="选择日期" value-format="timestamp" style="width:90px;" />
        <b>-</b>
        <el-date-picker v-model="end_time" type="date" placeholder="选择日期" value-format="timestamp" style="width:90px;" />
      </div>
      <div class="spsty2">
        <span style="">出库类型：</span>
        <el-select
      ...
      <div class="spsty1">
        产品：
        <el-input v-model="listQuery.product_name" type="text" clearable placeholder="请输入产品" style="width:90px;margin-right:15px;" />
      </div>
      <el-button
        v-waves
        class="filter-item"
    ...
    >
      ...
      <el-table-column label="出库类型" align="center" width="150" prop="stock_type">
        <template slot-scope="scope">
...
<script>
import { stockList } from '@/api/ManageGoods/PutOutStorage'
  ...
  methods: {
    ...
    getList() {
      ...
      if (this.bfirst) {
        ...
        stockList(blistQuery).then(response => {
